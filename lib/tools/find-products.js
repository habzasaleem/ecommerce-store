...
export const findProducts = tool({
  ...
  execute: async ({ query, maxPrice, category }) => {
    ...
    // Ask something containing the word "sabotage-json" (e.g. "find me
    // sabotage-json items") to force a genuine JSON parse failure, so
    // you can screen-record the output-error state catching a broken
    // tool response instead of a raw crash.
